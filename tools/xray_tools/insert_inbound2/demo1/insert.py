...
def insert_records(node_name, start_range, end_range):
    ...
    try:
        ...
        for i in range(start_range, end_range + 1):
            # 测试插入ss
            insert_shadowsocks(count, node_name, cursor)
            count += 1
            # 测试插入 vmess
            insert_vmess(count, node_name, cursor)
            count += 1
            # 测试插入 socks5
            insert_socks5(count, node_name, cursor)
            count += 1
        # 提交事务并关闭连接
        conn.commit()
        conn.close()
...
def insert_shadowsocks(i, node_name, cursor):
    sql = (
...
def insert_vmess(i, node_name, cursor):
    sql = (
        'INSERT INTO inbounds (id, user_id, up, down, total, remark, enable, expiry_time, listen, port, protocol, settings, stream_settings, tag, sniffing) '
        'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    characters = string.ascii_letters + string.digits
    ...
    cursor.execute(sql, (
        i + 1, 1, 0, 0, 0, node_name + str(i), 1, 0, "", 30000 + i, "vmess", json.dumps(setting_data),
        json.dumps(stream_data, indent=2), tag, json.dumps(sniffing, indent=2),))

def insert_socks5(i, node_name, cursor):
    sql = (
        'INSERT INTO inbounds (id, user_id, up, down, total, remark, enable, expiry_time, listen, port, protocol, settings, stream_settings, tag, sniffing) '
        'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    characters = string.ascii_letters + string.digits
    # 随机id生成：8+4+4+4+12
    random_pass = ''.join(random.choices(characters, k=10))
    setting_data = {
        "auth": "password",
        "accounts": [
            {
    ...
    cursor.execute(sql, (
        i + 2, 1, 0, 0, 0, node_name + str(i), 1, 0, "", 40000 + i, "socks", json.dumps(setting_data),
        json.dumps(stream_data, indent=2), tag, json.dumps(sniffing, indent=2),))
