...
def insert_records(node_name, start_range, end_range):
    ...
    try:
        ...
        for i in range(start_range, end_range + 1):
            ...
            random_email1 = ''.join(random.choices(characters, k=4))
            ...
            cursor.execute(sql, (
                count, 1, 0, 0, 0, node_name + str(i), 1, 0, "", 20000 + i, "shadowsocks", json.dumps(setting_data1),
                json.dumps(stream_data1, indent=2), tag1, json.dumps(sniffing1, indent=2),))
            count += 1

            # 测试插入 vmess ------------------------------------------------------
            random_id2 = generate_uuid()
            random_email2 = ''.join(random.choices(characters, k=4))
            ...
            cursor.execute(sql, (
                count, 1, 0, 0, 0, node_name + str(i), 1, 0, "", 30000 + i, "vmess", json.dumps(setting_data2),
                json.dumps(stream_data2, indent=2), tag2, json.dumps(sniffing2, indent=2),))
            count += 1

            # 测试插入 socks5 ------------------------------------------------------------
            sql3 = (
                'INSERT INTO inbounds (id, user_id, up, down, total, remark, enable, expiry_time, listen, port, protocol, settings, tag, sniffing) '
                'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
            ...
            cursor.execute(sql3, (
                count, 1, 0, 0, 0, node_name + str(i), 1, 0, "", 40000 + i, "socks", json.dumps(setting_data3),
                tag3, json.dumps(sniffing3, indent=2),))
            count += 1
        # 提交事务并关闭连接
        conn.commit()
        conn.close()
